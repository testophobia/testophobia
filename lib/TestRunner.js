  ...
  async _navigateToPage() {
    ...
    while (navAttempts > 0) {
      try {
        ...
        if (this.test.waitOnTarget) await this.browser.waitForTarget(this.test.waitOnTarget);
        else if (this.config.loadExtraDelay) await new Promise(r => setTimeout(r, this.config.loadExtraDelay));
        return;
      } catch (err) {
        if (navAttempts > 1) {
