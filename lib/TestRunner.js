...
const {
  generateScreenshot,
  writeGoldensManifest
} = require("./utils/generate-screenshot");
...
exports.TestRunner = class TestRunner {
  ...
  async _executeScreenshots(screenType, testRouteName) {
    ...
    this.output.prependDebugMessageToSpinner(
      `Generating Screenshot - ${testRouteName} (${screenType})`,
      false,
      this.testResults
    );
  ...
  async _navigateToPage() {
    ...
    const url = `${this.config.baseUrl}${
      this.test.path ? "/" + this.test.path.replace(/^\/+/g, "") : ""
    }`;
    this.output.prependDebugMessageToSpinner(
      ` - url: ${url}`,
      false,
    ...
    while (navAttempts > 0) {
      try {
        ...
        if (navAttempts > 1) {
          this.output.prependDebugMessageToSpinner(
            'baseUrl supplied cannot be reached. Retrying...',
            false,
            this.testResults
          );
        } else {
          this.output.displayFailure(
            `baseUrl supplied cannot be reached.\n${err.stack}`
          );
        }
        navAttempts--;
      }
  ...
  async _handleScreenshots(
    ...
    if (this.config.golden)
      writeGoldensManifest(`${this.dirToUse}/${fileName}`, this.test);
    if (!this.test.skipScreen) {
      await generateScreenshot(
        ...
        this.browser,
        this.config.quality,
        msg => this.output.displayFailure(msg)
      );
      if (!this.config.golden) {
        const filePath = `${
          this.dimensions.type
        }/${testRouteName}/${getIntialFileName(true)}.${this.config.fileType}`;
    ...
    if (this.test.actions) {
      await asyncForEach(this.test.actions, async (b, i) => {
        clipRegion =
          getActionClipRegion(b, this.test, screenType) || clipRegion;
        await this._handleActionScreenshots(
          `${fileName}`,
          b,
          dimensions,
          clipRegion,
  ...
  async _runComparison(testRouteName, filePath, action = false, index = false) {
    let resp = await this._compareScreenshots(
      testRouteName,
      filePath,
      action,
      index
  ...
  _compareScreenshots(testRouteName, filePath, action, index) {
    this.output.prependDebugMessageToSpinner(
      `Comparing Screenshots - ${testRouteName} (${this.dimensions.type})`,
      false,
      this.testResults
    );
    const testDefPath = !isNaN(this.test.inlineIndex)
      ? this.test.inlineIndex
      : this.test.testDefinitionPath;
    const sc = new ScreenCompare(
      this.config,
      this.test,
      testDefPath,
      this.dimensions,
  ...
  _getPath(screenType, route) {
    ...
    const path = `${this.dirToUse}/${fileName}/${getIntialFileName(false)}.${
      this.config.fileType
    }`;
  ...
  async _handleActionScreenshots(
    f,
    action,
    dimensions,
    clipRegion,
    ...
    if (
      ...
      action.excludeDimensions.includes(screenType)
    )
      return;
    if (!action.target) {
    ...
    try {
      ...
      this.output.displayFailure(
        `Issue performing action: ${action.type} for test: ${this.test.name}`
      );
    }
    if (!action.skipScreen) {
      const fileName = getActionFileName(actionIndex, action);
      this.output.prependDebugMessageToSpinner(
        `Generating Screenshot - ${testRouteName} (${fileName})`,
        false,
        this.testResults
      );
  ...
  _readGoldenDir(fileName) {
    ...
    this.existingGoldens = fs.readdirSync(dir).reduce((files, file) => {
      ...
      return isDirectory
        ? files
        : [...files, path.relative(this.config.goldenDirectory, name)];
    }, []);
  }
    ...
    if (this.test.actions) {
      ...
      this.test.actions.forEach((a, i) => {
        ...
        if (actionNames.includes(name)) {
          this.output.displayFailure(
            "Duplicate action description: " + getActionFileName(i, a, true)
          );
          return;
        }
