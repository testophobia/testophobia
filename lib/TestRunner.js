...
exports.TestRunner = class TestRunner {
  ...
  async _handleScreenshots(path, fileName, screenType, testRouteName, dimensions) {
    ...
    let clipRegion = this.test.clipRegion || this.config.clipRegion;
    ...
    if (this.test.actions) {
      clipRegion = this.test.actionsClipRegion || clipRegion;
      await asyncForEach(this.test.actions, async (b, i) => {await this._handleActionScreenshots(`${fileName}`, b, dimensions, clipRegion, testRouteName, screenType, i);});
    }
    if (this._checkBail()) {
      this.output.updateSpinnerDisplay(true, this.testResults, true);
      return this.testResults;
  ...
  async _handleActionScreenshots(f, action, dimensions, clipRegion, testRouteName, screenType, actionIndex) {
    ...
    if (!action.skipScreen) {
      ...
      const path = `${this.dirToUse}/${f}/${actionIndex}-${action.type}-${targetName}-screen.${this.config.fileType}`;
      const clip = action.clipRegion || clipRegion;
      await generateScreenshot(path, dimensions, clip, this.config.fileType, this.browser, this.config.quality, (msg) => this.output.displayFailure(msg));
      if (!this.config.golden) {
        const filePath = `${this.dimensions.type}/${testRouteName}/${actionIndex}-${action.type}-${targetName}-screen-scaled.${this.config.fileType}`;
        this.existingGoldens.splice(this.existingGoldens.indexOf(filePath), 1);
