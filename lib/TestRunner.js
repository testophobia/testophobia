...
exports.TestRunner = class TestRunner {
  ...
  async _initBrowser(dimensions) {
    let options = {debug: this.config.debug, dimensions, defaultTime: this.config.defaultTime, baseDelay: this.config.delay, delayModifier: this.config.delayModifier, pageLoadMax: this.config.pageLoadMax};
  ...
  async _navigateToPage() {
    ...
    try {
      if (this.output) this.output.prependDebugMessageToSpinner(new Date().getTime(), false, this.testResults);
