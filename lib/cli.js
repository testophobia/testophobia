...
exports.run = async config => {
  ...
  cli.flags.usePath ? _handlePathRoutes(cli, cli.flags.golden) :
    _executeTests(cli.flags.golden, useRoutes);
...
const _handlePathRoutes = (cli, goldenTest) => {
  let routes = [];
  ...
  cli.input[0].split(" ").forEach(r => glob(r, async (er, files) => {
    ...
    _executeTests(goldenTest, routes);
  }));
};
