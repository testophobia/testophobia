...
exports.Testophobia = class Testophobia {
  ...
  async run(testPath = false) {
    ...
    await this._executeTests(tests);
  }
  ...
  async _executeTests(tests) {
    ...
    if (parallel && this._testRuns.length) {
      ...
      this._testsComplete({tests: this._testRuns.map(t => t.name), failures: this.testResults, bailTriggered: this.config.bail && Boolean(this.testResults.length)});
    }
  }
  ...
  _runTestForDimension(test, testRouteName, d) {
    return new Promise(async resolve => {
      ...
      if (this._isParallel())
        this._nextParallelTest(resolve);
      else
        resolve();
    });
  }
