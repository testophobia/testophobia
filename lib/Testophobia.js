  ...
  async _executeTests(tests) {
    ...
    await asyncForEach(tests, async test => {
      ...
      await asyncForEach(dimensions, async d => {
        if (this._isParallel()) {
          this._testRuns.push({test: test, route: testRouteName, dimension: d});
        } else {
    ...
    if (this._isParallel() && this._testRuns.length) {
      this._testRuns.splice(0, this.config.maxParallel).forEach(r => {
