...
exports.Testophobia = class Testophobia {
  ...
  _clearTestophobiaDirectories(path = false) {
    if (path) {
      ...
      this.output.displayWarning(`Testophobia directory ${path} cleared.`);
      return 0;
    }
    return Promise.all([
  ...
  _goldenCheck() {
    if (!this.config.golden && !fs.existsSync(`${this.config.goldenDirectory}`)) {
      this.output.displayFailure(figures.cross + '  No Golden Images to Compare.');
      return 1;
    }
    return 0;
  }
  ...
  _generateScreenshots(test, testRouteName, d) {
    if ((this.config.bail && this.testResults.length)) return;
    const sg = new TestRun(this.config, test, d.type, this.dirToUse, this.testResults, this.output);
  ...
  _compareScreenshots(test, dimension, testRouteName) {
    ...
    const sc = new ScreenCompare(this.config, test, dimension, this.output);
  ...
  _handleSpinner(finished = false) {
    ...
    finished ? this.output.displayTestCompletion(bailTriggered, this.testResults) : this.output.updateSpinnerDisplay(bailTriggered, this.testResults, true);
  }
