  ...
  async _executeTests(tests) {
    ...
    await asyncForEach(tests, async test => {
      ...
      await asyncForEach(dimensions, async d => {
        if (this.config.maxParallel > 1) {
          testRuns.push({test: test, route: testRouteName, dimension: d});
        } else {
    ...
    if (this.config.maxParallel > 1) {
      while (testRuns.length) {
        const subset = testRuns.splice(0, this.config.maxParallel);
        await Promise.all(subset.map(r => this._runTestForDimension(r.test, r.route, r.dimension)));
      }
    }
