...
exports.Testophobia = class Testophobia {
  ...
  _checkFlagsAndFiles() {
    ...
    if (init) return generateConfigFile(this.config, this.cfgDirectory, message => this.output.displayMessage(message));
  ...
  async _createDirectories(testRouteName, dimensions, excludes) {
    ...
    this.testRunRootDir = this.config.golden ? this.config.goldenDirectory : this.config.testDirectory;
    await asyncForEach(dimensions, d => {
      if (!excludes || !excludes.includes(d.type)) {
        ...
        const dirToCreate = `${this.testRunRootDir}/${d.type}/${testRouteName}`;
  ...
  _runTestForDimension(test, testRouteName, d) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const tr = new TestRunner(++this._runnerCount, this.config, test, d, this.testRunRootDir, this.testResults, this.output);
