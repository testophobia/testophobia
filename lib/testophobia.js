...
exports.Testophobia = class Testophobia {
  ...
  async run() {
    ...
    let useRoutes = this._getRoutes(cli);

    if (!useRoutes) this._throwFileError();
    ...
    await cli.flags.usePath ? this._handlePathRoutes(cli, cli.flags.golden) :
      this._executeTests(cli.flags.golden, useRoutes);
  ...
  _getRoutes(cli) {
    return cli.input.length > 0
      ...
      : this.config.hasOwnProperty("routes")
        ? this.config.routes
        : false;
  }
  ...
  _handlePathRoutes(cli, goldenTest) {
    let routes = [];
    ...
    cli.input[0].split(" ").forEach(r => glob(r, async (er, files) => {
      ...
      this._executeTests(goldenTest, routes);
    }));
  }

  async _executeTests(goldenTest, routes) {
    const sg = new ScreenGenerator(this.config, goldenTest, routes);
    await sg.run();
    if (!goldenTest) {
      const sc = new ScreenCompare(this.config, routes);
