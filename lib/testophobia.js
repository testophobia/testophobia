...
exports.Testophobia = class Testophobia {
  ...
  async run() {
    ...
    await cli.input.length ? this._handleTestPaths(cli, cli.flags.golden) :
  ...
  _throwFileError() {
    global.response = 1;
    throw new Error(chalk.red("No test files found! Check your config or input path."));
  }
  ...
  _handleTestPaths(cli, goldenTest) {
    ...
    cli.input[0].split(" ").forEach(r => glob(r, async (er, files) => {
      ...
      tests.length ? this._executeTests(goldenTest, tests) : this._throwFileError();
    }));
  }
