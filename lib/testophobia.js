...
exports.Testophobia = class Testophobia {
  ...
  async run(testPath = false) {
    ...
    if (!this.config.tests && !testPath) this._throwFileError();

    await this._handleTestPaths(cli, testPath);
  ...
  async _handleTestPaths(cli, testPath) {
    let target = testPath ? testPath : cli.input[0] ? cli.input[0] : this.config.tests[0];
    // Do better check here 
    if (target.name) return this._executeTests(cli.flags.golden, this.config.tests);
    let testPaths = await glob.sync(target);
    await this._populateTests(testPaths, cli.flags.golden);
  }

  async _populateTests(testPaths, goldenTest) {
    let tests = [];
