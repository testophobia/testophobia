...
exports.Testophobia = class Testophobia {
  ...
  async run(testPath = false) {
    ...
    if (!this.config.golden && !fs.existsSync(`${this.config.goldenDirectory}`)) return this._exit('Error: No Golden Images to Compare.');
    let resp = await this._handleTestPaths(target, this.config.golden);
    if (resp.goldenError) return this._exit('Error: Missing Golden Images to Compare.');
  ...
  _handleConfigs() {
    ...
    try {
      this.config = Object.assign(loadConf(this.config), this.cli.flags);
    } catch (error) {
      confError = error;
    }
    if (confError) {
