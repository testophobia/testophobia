...
exports.Testophobia = class Testophobia {
  ...
  async run(testPath = false) {
    let target = await this._handleConfigs(testPath);
    ...
    if (!this.config.golden && resp.failures.length) this._handleFailures(resp);
    return global.response;
  }

  _handleConfigs(testPath) {
    let passedConfigTests = this.config.tests || false;
    let confError = null;
    const cli = this._configureCliCommands();
    try {
      this.config = Object.assign(loadConf(this.config), cli.flags);
    } catch (error) {
      confError = error;
    }
    if (confError) {
    ...
    return testPath ? testPath : passedConfigTests ? passedConfigTests : cli.input[0] ? cli.input[0] : this.config.tests;
  }
  ...
  _printWelcomeMessage() {
    log.info('😱 Starting Testophobia...', 'cyan');
    log.debug('config -----------------------------------');
  ...
  launchViewer() {
    ...
    log.info('🔍 Testophobia Viewer served on port 8090', 'cyan');
