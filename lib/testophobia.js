...
exports.Testophobia = class Testophobia {
  ...
  async run(testPath = false) {
    ...
    if (!this.config.golden) resp.find(r => r.response > 0) ? this._handleFailures(resp) : this._handleSuccess();
  ...
  _handleFailures(resp) {
    console.log('some tests fail!');
    this._createResultFile(resp);
  }
  ...
  _createResultFile(resp) {
    ...
    fs.appendFileSync(this.resultFile, JSON.stringify(Object.assign({}, this.results, {failures: resp})), err => {
      if (err) console.log("There was an error adding info to the JSON file");
    });
    this._displayErrorDetails(resp);
  }

  _displayErrorDetails(res) {
    res.forEach(r => r.failures.length ? r.failures.forEach(f => {
      console.log(
        chalk.red(
