...
exports.Testophobia = class Testophobia {
  ...
  async run(testPath = false) {
    ...
    let target = testPath ? testPath : passedConfig.tests ? passedConfig.tests : cli.input[0] ? cli.input[0] : this.config.tests;

    await this._handleTestPaths(target, cli.flags.golden);
  ...
  async _handleTestPaths(target, isGolden) {
    ...
    await this._populateAndRunTests(testPaths, isGolden);
  }
