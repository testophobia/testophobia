...
exports.Testophobia = class Testophobia {
  ...
  async run(testPath = false) {
    ...
    if (!this.config.golden && !fs.existsSync(`${this.config.goldenDirectory}`)) return log.fatal('Error: No Golden Images to Compare.');
  ...
  _generateConfigFile() {
    log.info('😱 Generating Testophobia config...', 'cyan');
    try {
      ...
      log.info(chalk.green('✔') + '  Done.', 'cyan');
    }
