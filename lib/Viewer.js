...
export class Viewer {
  ...
  async init(config) {
    return new Promise(async (resolve, reject) => {
      ...
      this.resultsDir = this.config.diffDirectory + '/' + this.config.currentBrowser;
      resolve();
    });
  }
   ...
   */
  async handleTestResults(testResults, keepFailures) {
    await generateResultFile(this.config, testResults, keepFailures);
  ...
  /* istanbul ignore next */
  launchViewer() {
    this.output.displayViewerMessage(this.resultsDir);
    serveViewer(this.config, this.resultsDir);
  }
};
