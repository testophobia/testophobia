...
export class Output {
  ...
  /**
   * Get the test paths for all tests to be run
   *
   * @param {array} tests Array of tests to get paths for
   */
  getTestPaths(tests) {
    let paths = '';
    tests.forEach(t => {
      paths += t.testDefinitionPath + '\n';
    });
    return paths;
  }

  showStats(extraOutput) {
    this._getLog().info('   ' + chalk.green(`${this.total} tests found`), 'cyan');
    if (extraOutput) this._getLog().info(extraOutput);
  }
