...
exports.Output = class Output {
  ...
  _resolveDimensionCounts(test) {
    return test.hasOwnProperty('dimensions') ? test.dimensions.length + this.config.dimensions.length : this.config.dimensions.length;
  }

  _resolveActionCounts(test) {
    let count = 0;
    this.dimensions = test.hasOwnProperty('dimensions') ? [...this.config.dimensions, ...test.dimensions] : this.config.dimensions;
