...
exports.ScreenGenerator = class ScreenGenerator extends ScreenBase {
  ...
  async run() {
    ...
    await asyncForEach(this.tests, async t => {
      ...
      if (!this.isGolden) {
        let resp = await this.compareScreenshot();
    ...
    return {tests: this.tests.map(t => t.name), failures: this.testResults, bailTriggered: bailTriggered};
  }
  ...
  updateSpinnerDisplay(bailTriggered) {
    ...
    let prefix = chalk.cyan((pending) ? 'Running Tests' : 'Testing Complete');
    if (bailTriggered) prefix = chalk.cyan('Bailed');
    this.spinner.text = ` ${prefix} [${chalk.green(`${passed} passed`)} | ${chalk.red(`${failed} failed`)}${(pending) ? ` | ${pending} pending` : ''}]`;
  }
  ...
  compareScreenshot() {
    const sc = new ScreenCompare(this.initialConfig, this.tests, this.test);
    return sc.run();
  }
  ...
  async takeScreenshot(view, testRouteName, dimensions) {
    ...
    if (this.test.actions) {
      await asyncForEach(this.test.actions, async b => {await this.handleAction(`${fileName}`, b, dimensions);});
    }
  }
  ...
  async handleAction(f, b, dimensions) {
    ...
    try {
      ...
      global.response = 1;
      //TODO: Output error message detailing failed action here
    }
    let options = {
      ...
      if (element) {
        switch (config.action) {
          ...
          case 'hover':
            return element;
          case 'scroll':
            return element.scrollTop ? element.scrollTop = config.scrollTop : element.scrollLeft = config.scrollLeft;
          default:
            element[config.action]();
        }
      }
    }, {el: b.target, action: b.type, property: b.property, value: b.value, scrollTop: b.scrollTop, scrollLeft: b.scrollLeft, attribute: b.attribute});
    if (b.type === 'hover') {
      res.asElement().hover();
