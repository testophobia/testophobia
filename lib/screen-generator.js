...
exports.ScreenGenerator = class ScreenGenerator extends ScreenBase {
  ...
  async run() {
    ...
    return {tests: this.tests.map(t => t.name), failures: this.testResults, bailTriggered: this.bail && Boolean(this.testResults.length)};
  }
  ...
  updateSpinnerDisplay() {
    ...
    this.spinner.text = `Generating ${this.isGolden ? 'golden' : 'test'} screens ${this.currentTest}/${total} ...`;
  }
  ...
  handleEndDisplay(start) {
    ...
    let message = chalk.cyan(
      ` Finished generating ${this.isGolden ? 'golden' : 'test'} screens! (${(end / 1000).toFixed(1)}s)`
    );
