...
exports.ScreenGenerator = class ScreenGenerator extends ScreenBase {
  ...
  async run() {
    let goldenError = false;
    this.currentTest = 0;
    this.spinner = new ora();
    this.updateSpinnerDisplay();
    await asyncForEach(this.tests, async t => {
      if ((this.bail && this.testResults.length) || goldenError) return;
      let testRouteName = t.name.split('.')[0];
      this.test = t;
      goldenError = await this.createDirectories(testRouteName);
      if (!goldenError) await asyncForEach(this.dimensions, async d => {
        ++this.currentTest;
    ...
    return {tests: this.tests.map(t => t.name), failures: this.testResults, bailTriggered, goldenError};
  }
