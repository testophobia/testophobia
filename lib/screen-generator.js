...
exports.ScreenGenerator = class ScreenGenerator extends ScreenBase {
  ...
  async takeScreenshot(view, testRouteName, dimensions) {
    ...
    await this.page.goto(
      `${this.baseUrl}/${this.test.path ? this.test.path : testRouteName}`, {waitUntil: 'networkidle0'}
    );
    let options = {
  ...
  async _optimizeImages(path, dimensions) {
    const oldPath = path;
    const width = dimensions.scale ? Math.floor(dimensions.width * (dimensions.scale / 100)) : dimensions.width;
    const height = dimensions.scale ? Math.floor(dimensions.height * (dimensions.scale / 100)) : dimensions.height;
    await sharp(path)
      .resize(width, height, {
