...
exports.ScreenGenerator = class ScreenGenerator extends ScreenBase {
  ...
  async _optimizeImages(path, dimensions) {
    ...
    const width = dimensions.scale ? Math.floor(dimensions.width * (dimensions.scale / 100)) : Math.floor(dimensions.width / dimensionsRatio);
    const height = dimensions.scale ? Math.floor(dimensions.height * (dimensions.scale / 100)) : Math.floor(dimensions.height / dimensionsRatio);
    await sharp(path)
      .resize(width, height, {
