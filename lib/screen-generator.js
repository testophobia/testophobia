...
exports.ScreenGenerator = class ScreenGenerator extends ScreenBase {
  ...
  async run() {
    ...
    await this.asyncForEach(this.routes, async (f, i) => {
      let routeName = f.split(".")[0];
      this.route = global.conf.routes ? global.conf.routes[i] : f;
  ...
  calculateTotalTests() {
    let total = 0;
    if (global.conf.routes) global.conf.routes.map(r => total += r.behaviors ? r.behaviors.length + 1 : 1);
    else total += this.routes.length;
  ...
  async takeScreenshot(view, routeName, dimensions) {
    ...
    if (global.conf.routes && this.route.behaviors) {
