/* global global, require, querySelectorShadowDom, exports */
...
const {Logger, resolveNodeModulePath, asyncForEach} = require('./utils');
...
exports.ScreenGenerator = class ScreenGenerator extends ScreenBase {
  ...
  async addSelectorScript() {
    let pathToUse = await resolveNodeModulePath("query-selector-shadow-dom");
    return this.page.addScriptTag({
      path: path.join(
        pathToUse,
        "query-selector-shadow-dom/dist/querySelectorShadowDom.js"
      )
    });
  }
  ...
  async performAction(b) {
    ...
    if (b.type === 'hover' || b.type === 'click') {
      res.asElement().hover();
      if (res.focus) res.focus();
      if (b.type === 'click') res.click();
    }
    if (b.type === 'click') {
      try {
