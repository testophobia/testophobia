  ...
  let res = await page.evaluateHandle(config => {
    ...
    let root =
      targetParts.length > 1
        ? querySelectorShadowDom.querySelectorDeep(targetParts[0])
            .contentDocument
        : window.document;
    ...
    if (element) {
      switch (config.type) {
        ...
        case "scroll":
          config.scrollTop
            ? (element.scrollTop = config.scrollTop)
    ...
    if (config.blurActiveElement && document.activeElement)
  ...
  if (el) {
    switch (action.type) {
      ...
      case "click":
        ...
        try {
          let center = await getElementCenter(el);
          if (action.hideMouse) await page.mouse.move(-100, -100);
          else if (center) await page.mouse.move(center.x, center.y);
        } catch (err) {
          throw new Error(
            `Unable to move mouse to element for ${test.name} - ${action.type} - ${action.target}`
          );
        }
        break;
      case "triggerOpenFileDialog":
      ...
      case "drag":
        try {
          ...
          throw new Error(
            `Unable to drag element for ${test.name} - ${action.type} - ${action.target}`
          );
        }
        break;
    }
...
const getElementCenter = async el => {
