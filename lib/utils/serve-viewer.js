...
const {readTestFile} = require('../utils/format-tests');
const {writeGoldensManifest} = require('../utils/generate-screenshot');
const {getGoldenImagesForViewer} = require('../utils/get-goldens-for-viewer');
...
exports.serveViewer = (conf, resultsPath) => {
  ...
  app.get('/results.json', (req, res) => {
    ...
    let result;
    if (conf.golden) {
      const imageArr = getGoldenImagesForViewer(conf);
      result = {golden:true, images:imageArr};
    } else {
      result = resultsJSON;
    }
  ...
  app.post('/apply-golden/:testIdx', (req, res) => {
    ...
    const testDef = (!isNaN(testDefPath)) ?
                    conf.tests[testDefPath] :
                    readTestFile(testDefPath).default;
