...
const {readTestFile} = require('../utils/test/format-tests');
const {writeGoldensManifest} = require('../utils/generate-screenshot');
const {getGoldenImagesForViewer, getGoldenDirectoriesForViewer} = require('../utils/get-goldens-for-viewer');
...
exports.serveViewer = (conf, resultsPath) => {
  ...
  app.post('/apply-golden/:testIdx', (req, res) => {
    ...
    const testDef = !isNaN(testDefPath) ? conf.tests[testDefPath] : readTestFile(testDefPath).default;
