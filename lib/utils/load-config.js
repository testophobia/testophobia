...
function loadConfigFile(projectDir, packageConf) {
  let fileConf;
  try {
  ...
  if (fileConf === MISSING_DEFAULT_EXPORT) {
    throw new Error('testophobia.config.js must have a default export, using ES module syntax');
  }

  if (fileConf !== NO_SUCH_FILE) {
    if (Object.keys(packageConf).length > 0) {
      throw new Error('Conflicting configuration in testophobia.config.js and package.json');
    }
    ...
    if (!isPlainObject(fileConf) && typeof fileConf !== 'function') {
      throw new TypeError('testophobia.config.js must export a plain object or factory function');
    }

    if (typeof fileConf === 'function') {
      fileConf = fileConf({projectDir});
      if (fileConf && typeof fileConf.then === 'function') {
        throw new TypeError('Factory method exported by testophobia.config.js must not return a promise');
      }
      if (!isPlainObject(fileConf)) {
        throw new TypeError('Factory method exported by testophobia.config.js must return a plain object');
      }
    }
  }
  return fileConf;
...
function loadConfig(passedDefaults = {}) {
  ...
  let projectDir = testConfig || filepath === null ? process.cwd() : path.dirname(filepath);
  return Object.assign({}, configDefaults, loadConfigFile(projectDir, packageConf), passedDefaults, packageConf, {projectDir});
}
module.exports = {
  loadConfig,
  loadConfigFile,
