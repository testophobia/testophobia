...
export const performAction = async (action, page, test) => {
  const targetElement = await page.waitForSelector(action.target, {state:'attached'});
    ...
    if (config.element) {
      switch (config.action.type) {
        ...
        case 'keypress':
          ...
          events.forEach(e => {
            e.key = config.action.key;
            e.which = e.key;
            config.element.dispatchEvent(e);
          });
          returnEl = config.element;
          break;
        case 'mouseevent':
        ...
        try {
          if (!!action.dropTarget) {
            ...
            if (el) {
              ...
              await el.hover();
              await page.mouse.down();
              await page.mouse.move(center.x + (action.x || 0), center.y + (action.y || 0));
