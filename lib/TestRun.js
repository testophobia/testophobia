...
exports.TestRun = class TestRun {
  ...
  async _navigateToPage() {
    ...
    if (this.output) this.output.prependDebugMessageToSpinner(` - url: ${url}`, false, this.testResults, true);
    try {
  ...
  async _runComparison(testRouteName, filePath, action = false) {
    let resp = await this._compareScreenshots(testRouteName, filePath, action);
  ...
  _compareScreenshots(testRouteName, filePath, action) {
    this.output.prependDebugMessageToSpinner(`Comparing Screenshots - ${testRouteName} (${this.dimensions.type})`, false, this.testResults, true);
    const sc = new ScreenCompare(this.config, this.test, this.dimensions);
    return sc.run(testRouteName, filePath, action);
  }
  ...
  async _handleActionScreenshots(f, b, dimensions, testRouteName, screenType, actionIndex) {
    ...
    if (!b.skipScreen) {
      if (this.output) this.output.prependDebugMessageToSpinner(`Generating Screenshot - ${testRouteName} (${screenType}-${b.type}-${targetName})`, false, this.testResults, true);
      ...
      await this._runComparison(testRouteName, filePath, b);
