...
exports.Browser = class Browser {
  ...
  async launch(config, dimensions, testRouteName) {
    try {
      this.browser = await playwright.chromium.launch(this._getPlaywrightConfig(config.debug));
      ...
      await this._initHandlers(config.debug, config.verbose, config.defaultTime, testRouteName);
  ...
  async _initHandlers(debug, verbose, defaultTime, testRouteName) {
    this.page.on('dialog', dialog => dialog.accept()); //TODO - someday figure out to give tests control over this
    if (debug || verbose) this.page.on('console', msg => this.output.displayDebugMessage(`\nCHROMIUM LOG: ${msg.text()}`));
