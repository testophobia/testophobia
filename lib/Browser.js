...
export class Browser {
  ...
  async goto(url) {
    if (this.pageLoadMax) {
      await Promise.race([this.page.goto(url, {waitUntil: 'load'}), new Promise(x => setTimeout(x, this.pageLoadMax).unref())]);
    } else {
  ...
  async goBack() {
    if (this.pageLoadMax) {
      await Promise.race([this.page.goBack({waitUntil: 'load'}), new Promise(x => setTimeout(x, this.pageLoadMax).unref())]);
    } else {
  ...
  async waitForTarget(target) {
    await this.page.waitForSelector(target, {state:'attached'});
  }
