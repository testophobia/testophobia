  ...
  async run() {
    ...
    if (this.test.actions) {
      await asyncForEach(this.test.actions, async (b, i) => {
        ...
        let res = retrieveImages(filePath, this.testDirectory, this.goldenDirectory, this.fileType);
        if (res.error) return;
        pixDiff = await this._compareScreenshots(res.img1, res.img2);
    ...
    let resp = retrieveImages(filePath, this.testDirectory, this.goldenDirectory, this.fileType);
    //TODO: Logger, need to error out here, golden image issue (eventually will return error to test runner)
    if (resp.error) return;
    pixDiff = await this._compareScreenshots(resp.img1, resp.img2);
