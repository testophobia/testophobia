...
exports.ScreenCompare = class ScreenCompare extends ScreenBase {
  ...
  async run() {
    ...
    await this.asyncForEach(this.routes, async (f, i) => {
      this.routeName = f.split(".")[0];
      this.file =
        global.conf.routes && global.conf.routes[i].behaviors
          ? global.conf.routes[i]
          : f;
      await this.asyncForEach(this.dimensions, async d => {
        this.testDimensions = d;
        if (this.file.behaviors) {
          this.file.behaviors.map(async b => {
  ...
  prepareImages(screenType, behavior = false, verbose) {
    ...
    this.filePath = `${this.screenType}/${this.routeName}/${
      ...
      }screen-compressed.${this.fileType}`;
    let img1, img2;
    if (verbose)
      console.log(
        chalk.dim(
