...
exports.ScreenCompare = class ScreenCompare {
  ...
  async _performComparison(filePath, action, index) {
    ...
    if (!img1 || !img2) {
      this._setFailure(
        ...
        action ? `${action.type}-${action.target}` : 'none',
        null,
        this.dimensions,
    ...
    const threshold = action.threshold || this.test.threshold || this.config.threshold;
  ...
  _handleScreenshotsDifferent(screenType, filePath, diff, dp, action, index) {
    let actionPath = action ? `${index}-${action.type}-${cleanTargetName(action.target)}` : '';
  ...
  _setFailure(test, screenType, action, pixelDifference, dimensions, testFileLocation, goldenFileLocation, diffFileLocation) {
    this.failure = {
      test,
      screenType,
      action,
