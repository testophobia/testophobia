...
(() => {
  ...
  function setSelectedElement() {
    Testophobia.chrome.devtools.inspectedWindow.eval(`(function(){return getUniqueSelector($0);}())`, {useContentScriptContext: true}, result => {
  ...
  function buildList() {
    ...
    rendered += '<option value="delay">Delay</option>';
    rendered += '</select>';
    rendered += '</div>';
    rendered += '<br><br>';
