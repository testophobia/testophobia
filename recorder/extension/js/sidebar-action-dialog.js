...
(() => {
  ...
  Testophobia.loadActionDialog = actionIdx => {
    ...
    Testophobia.dialogActionType =
      Testophobia.activeTest.actions[Testophobia.dialogActionIndex].type;
  ...
  function layoutDialog() {
    ...
    const addField = (label, prop) =>
      `\n      <label>${label}</label>\n      <input id="txt${prop}" value="${action[
        prop
      ] || ""}"/>`;
    switch (Testophobia.dialogActionType) {
      case "setProperty":
        fieldsHtml = `${addField("Property Name", "property")}${addField(
          "Property Value",
          "value"
        )}`;
        break;
      case "setAttribute":
        fieldsHtml = `${addField("Attribute Name", "attribute")}${addField(
          "Attribute Value",
          "value"
        )}`;
        break;
      case "removeAttribute":
        ...
        break;
      case "scroll":
        fieldsHtml = `${addField("Scroll Top", "scrollTop")}${addField(
          "Scroll Left",
          "scrollLeft"
        )}`;
        break;
      case "keypress":
        ...
        break;
      case "input":
        fieldsHtml = `${addField("Value", "value")}${addField(
          "Replace",
          "replace"
        )}`;
        break;
      ...
      case "triggerOpenFileDialog":
        fieldsHtml = `${addField("File Path", "filePath")}`;
        break;
    }
    $("#divAddlFields").html(fieldsHtml + "\n    ");
    $("#divActionProps #txtDescription").val(
      Testophobia.checkEmpty(action.description)
    );
    $("#divActionProps #txtDelay").val(Testophobia.checkEmpty(action.delay));
    $("#divActionProps #txtThreshold").val(
      Testophobia.checkEmpty(action.threshold)
    );
    loadClipRegions(action);
    loadExcludedDimensions(action);
    $("#divActionProps #chkSkipScreen").prop(
      "checked",
      action.skipScreen || false
    );
    $("#divActionProps #chkBlurElement").prop(
      "checked",
      action.blurActiveElement || false
    );
    $("#divActionProps #chkHideMouse").prop(
      "checked",
      action.hideMouse || false
    );
    $("#divBackdrop").removeAttr("hidden");
    $("#divActionDialog").removeAttr("hidden");
  ...
  function loadClipRegions(action) {
    Testophobia.buildListControl(
      ...
      action.clipRegions,
      f =>
        `${f.type} - ${f.left || 0}:${f.top || 0}:${f.width ||
          f.right ||
          "100%"}:${f.height || f.bottom || "100%"}`,
      e => {
        Testophobia.editingClipRegionForActionIndex = $(e.currentTarget).attr(
          "data-row"
        );
        Testophobia.hideActionsDialog();
  ...
  function saveEdits() {
    const action =
      Testophobia.activeTest.actions[Testophobia.dialogActionIndex];
    Testophobia.validation.validate(
      {
        name: "description",
        type: "string",
        selector: "#divActionProps #txtDescription",
        required: false
      },
      action
    );
    Testophobia.validation.validate(
      {
        name: "delay",
        type: "number",
        selector: "#divActionProps #txtDelay",
        required: false
      },
      action
    );
    Testophobia.validation.validate(
      {
        name: "threshold",
        type: "decimal",
        selector: "#divActionProps #txtThreshold",
        required: false
      },
      action
    );
    Testophobia.validation.validate(
      {
        name: "skipScreen",
        type: "boolean",
        selector: "#divActionProps #chkSkipScreen",
        required: false
      },
      action
    );
    ...
    Testophobia.activeTest.actions[Testophobia.dialogActionIndex].type =
      Testophobia.dialogActionType;
    Testophobia.activeTest.actions[Testophobia.dialogActionIndex].target = $(
      "#txtTarget"
    ).val();
  ...
  function setSelectedElement() {
    Testophobia.chrome.devtools.inspectedWindow.eval(
      `(function(){return getUniqueSelector($0);}())`,
  ...
  function addDimensionExclude() {
    ...
    rendered += "</div>";
    rendered +=
      '<button id="btnApplyValueEdit" class="dialogBtn green button">Apply</button>';
    ...
    Testophobia.showValueDialog(
      ...
      action.excludeDimensions,
      {
        ...
        selector: "#divValueEditDialog #divValueEditProps #txtValue",
        type: "string",
        required: true
      },
  ...
  function buildList() {
    ...
    rendered +=
      '<div class="dropdown button blue" title="Select an Action Type">';
    ...
    rendered +=
      '<option value="triggerOpenFileDialog">Open File Dialog</option>';
    ...
    rendered +=
      '<div class="listHeader"><label>Exclude Dimensions</label><button id="btnAddDimensionExclude" class="link-button">Add</button></div>';
    rendered += '<ul id="lstExcludedDimensions" class="dialogList"></ul>';
    rendered +=
      '<div class="listHeader"><label>Clip Regions (this action)</label><button id="btnAddClipRegionPerAction" class="link-button">Add</button></div>';
    rendered += '<ul id="lstClipRegionsPerAction" class="dialogList"></ul>';
    rendered += '<input id="chkSkipScreen" type="checkbox"/>';
    rendered +=
      '<label for="chkSkipScreen">Skip snapshot for this action</label>';
    rendered += "<br>";
    rendered += '<input id="chkBlurElement" type="checkbox"/>';
    rendered +=
      '<label for="chkBlurElement">Unfocus the active element before snapshot</label>';
    rendered += "<br>";
    rendered += '<input id="chkHideMouse" type="checkbox"/>';
    rendered +=
      '<label for="chkHideMouse">Hide the mouse before snapshot</label>';
    rendered += "</div>";
    rendered +=
      '<button id="btnSaveEdits" class="dialogBtn green button">Apply</button>';
  ...
  Testophobia.chrome.devtools.panels.elements.onSelectionChanged.addListener(
    setSelectedElement
  );
})();
