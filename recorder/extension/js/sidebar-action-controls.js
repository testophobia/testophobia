...
(() => {
  function buildView() {
    ...
    rendered += "</div>";
    rendered +=
      '<button id="btnAddAction" class="green button" title="Add Action">+</button>';
    rendered += "</div>";
    rendered += "<br/>";
    rendered +=
      '<label id="actionsLbl">Recorded Actions: <small>(Hover for full details)</small></label>';
    rendered += '<div id="actionsList"></div>';
    rendered +=
      '<button id="btnClearAll" class="red button" title="Clear All Actions">Clear All</button>';
    rendered +=
      '<button id="btnSaveTest" class="green button" title="Save the Test">Save</button>';
  ...
  $("#btnAddAction").click(() => {
    ...
    Testophobia.activeTest.actions.push({
      type: actionType,
      target: $("#divSelected").html()
    });
